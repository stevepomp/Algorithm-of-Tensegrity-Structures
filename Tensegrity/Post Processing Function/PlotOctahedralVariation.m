function PlotOctahedralVariation(Tensegrity)
...
% % % plot nodes
% method 1
[XS,YS,ZS] = sphere(20);rSphere = 0.02;
...
for i = Tensegrity.n_cable-5:Tensegrity.n_cable
    ie = Tensegrity.cable_index(i,1);ic=Tensegrity.cable_group(i,1);
    plot3(X(ie,:), Y(ie,:), Z(ie,:), 'm--', 'LineWidth', 3);
    if (LabelOn)
        text(mean(X(ie,:)), mean(Y(ie,:)), mean(Z(ie,:)), ...
            ['C' num2str(i)], 'FontSize', 16, 'Color', 'm');
    end
...
axis off;axis image;axis vis3d;%axis equal;
